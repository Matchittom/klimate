...
export const createProductReview = async (
  ...
  rating
   ...
   if (!reviewerName) throw 'Error: Input missing reviewerName!';
   if (!review) throw 'Error: Input missing review!';
   if (!rating) throw 'Error: Input missing rating!';
 
   if (typeof productId !== 'string' || typeof title !== 'string' || typeof reviewerName !== 'string' || typeof review !== 'string') throw 'Error: Given input must be a string!';
   if (productId.trim().length === 0 || title.trim().length === 0 || reviewerName.trim().length === 0 || review.trim().length === 0) throw 'Error: Cannot be empty string!';

   productId = productId.trim();
   if (!ObjectId.isValid(productId)) throw 'Error: Invalid object ID';

   if (typeof rating !== 'number' || rating < 1 || rating > 5) throw "Error: Rating must be a number greater than 0 and less than 5.1 AND must only have 2 decimal places";
   ...
   if (!ratingFunctionForDecimal(rating)) throw "Error: only allowed 1 decimal place"
...
if (insertInfo === null || !insertInfo) throw 'Error: No product with that id'; ///////////////////////////////////////////////
